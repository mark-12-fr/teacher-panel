...
export default function SectionDetailPage() {
  ...
  useEffect(() => {
    Promise.allSettled([loadDetails(), loadStudents()]).then(() => setLoading(false));
  }, [loadDetails, loadStudents]);

  // Live roster updates: a student added/edited/removed elsewhere (another
  // tab, or a facilitator) reflects here with no manual refresh. No-op if
  // realtime isn't enabled on the DB.
  useEffect(() => {
    let channel: any;
    try {
      channel = getSupabase()
        .channel("teacher-roster-" + sectionId)
  ...
  async function addStudent() {
    if (!addName.trim() || !addIdNo.trim()) return showToast("Please fill in all fields!", true);
    const entry = { id: "_opt_" + Date.now().toString(36), full_name: addName.trim(), id_no: addIdNo.trim() };
  ...
  return (
    <>
      ...
      <div className="class-list-box fade-swap" key={`${viewSemester}-${viewQuarter}`}>
      ...
      <div className="modal" style={{ display: addOpen ? "flex" : "none" }} onClick={(e) => e.target === e.currentTarget && setAddOpen(false)}>
        <div className="modal-content">
          ...
          <input type="text" placeholder="ID Number" value={addIdNo} onChange={(e) => setAddIdNo(e.target.value)} style={{ marginTop: 8 }} />
